...
Info
    Validator for the gen_mmap bundle options.
'''
...
class GenMmapBundleOptionsValidator:
    '''
        Validator for the gen_mmap bundle options.
    ...
    @classmethod
    def validate(cls, options: GenMmapBundleOptions) -> None:
        '''
            Validates the gen_mmap bundle options.

            :param options: The gen_mmap bundle options to be validated.
            :exceptions:
        ...
        '''
        ctx: str = 'gen_mmap_bundle_options_validator::validate(...)'
        msg_options_none: str = 'the gen_mmap bundle options must be provided'
        msg_options_istype: str = 'the gen_mmap bundle options must be a Mapping'

        not_none(options, ctx, msg_options_none)
        istype(options, Mapping, ctx, msg_options_istype)

        for attr_name, expected_type in GenMmapBundleKeys.get_option_to_type().items():

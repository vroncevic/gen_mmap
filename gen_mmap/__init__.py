...
__copyright__ = 'Copyright 2022, https://vroncevic.github.io/gen_mmap'
...
class GenMMAP(CfgCLI):
    '''
        ...
        It defines:
            ...
            :methods:
                ...
                | __str__ - dunder method for GenMMAP.
    '''

    __metaclass__ = CooperativeMeta
    GEN_VERBOSE = 'GEN_MMAP'
    CONFIG = '/conf/gen_mmap.cfg'
    LOG = '/log/gen_mmap.log'
    LOGO = '/conf/gen_mmap.logo'
    ...
    def __init__(self, verbose=False):
        ...
        gen_mmap_property = {
            ...
            'ats_repository': 'gen_mmap',
            'ats_name': 'gen_mmap',
            'ats_logo_path': '{0}{1}'.format(current_dir, GenMMAP.LOGO),
            'ats_use_github_infrastructure': True
        }
        splash = Splash(gen_mmap_property, verbose=verbose)
        base_info = '{0}{1}'.format(current_dir, GenMMAP.CONFIG)
        CfgCLI.__init__(self, base_info, verbose=verbose)
        verbose_message(GenMMAP.GEN_VERBOSE, verbose, 'init tool info')
        self.logger = ATSLogger(
            GenMMAP.GEN_VERBOSE.lower(),
            '{0}{1}'.format(current_dir, GenMMAP.LOG),
            verbose=verbose
        )
        if self.tool_operational:
            self.add_new_option(
                GenMMAP.OPS[0], GenMMAP.OPS[1],
                dest='gen', help='generate option'
            )
            self.add_new_option(
                GenMMAP.OPS[2], GenMMAP.OPS[3],
                action='store_true', default=False,
                help='activate verbose mode for generation'
            )
            self.add_new_option(
                GenMMAP.OPS[4], action='version', version=__version__
            )
    ...
    def process(self, verbose=False):
        ...
        if self.tool_operational:
            ...
            if num_of_args_sys > 1:
                ...
                if operation not in GenMMAP.OPS:
                    sys.argv.append('-h')
            else:
                sys.argv.append('-h')
            ...
            if not pro_exists:
                if bool(getattr(args, 'gen')):
                    print(
                        '{0} {1} [{2}]'.format(
                            '[{0}]'.format(GenMMAP.GEN_VERBOSE.lower()),
                            'generating', getattr(args, 'gen')
                        )
                    )
                    generator = GenMMAPSetup(verbose=verbose)
                    status = generator.gen_setup(
                        getattr(args, 'gen'), verbose
                    )
                    if status:
                        success_message(GenMMAP.GEN_VERBOSE, 'done\n')
                        ...
                        error_message(
                            GenMMAP.GEN_VERBOSE, 'generation failed'
                        )
                        self.logger.write_log(
                    ...
                    error_message(
                        GenMMAP.GEN_VERBOSE, 'provide project name'
                    )
                    self.logger.write_log(
                ...
                error_message(
                    GenMMAP.GEN_VERBOSE, 'project already exist'
                )
                self.logger.write_log(
            ...
            error_message(
                GenMMAP.GEN_VERBOSE, 'tool is not operational'
            )
            self.logger.write_log(

...
try:
    ...
    from mmap.mmap_setup import MMAPSetup
...
class GenMMAP(CfgBase):
    """
        Define class GenMMAP with attribute(s) and method(s).
    ...
    def __init__(self, verbose=False):
        """
            ...
            :exceptions: None
        """
        verbose_message(GenMMAP.VERBOSE, verbose, 'Initial configuration')
        current_dir = Path(__file__).resolve().parent
        base_config_file = "{0}{1}".format(current_dir, GenMMAP.__CONFIG)
        CfgBase.__init__(self, base_config_file, verbose=verbose)
        if self.tool_status:
            self.add_new_option(
                GenMMAP.__OPS[0], GenMMAP.__OPS[1], dest="pro",
                help="generate Memory Map project skeleton"
            )
    ...
    def process(self, verbose=False):
        ...
        if self.tool_status:
            ...
            if num_of_args == 1 and opts.pro and not project_exists:
                generator, gen_status = MMAPSetup(verbose=verbose), False
                message = "{0} {1} [{2}]".format(
                    "[{0}]".format(self.name),
                    'Generating Memory Map project skeleton', opts.pro
                )
                print(message)
                gen_status = generator.gen_mmap_setup("{0}".format(opts.pro))
                if gen_status:
            ...
            error_message('gen_mmap', 'Tool is not operational')
